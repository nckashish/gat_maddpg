...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        #act_pdtype_n = [env.action_space() for i in act_space_n]
        # set up placeholders
        #obs_ph_n = make_obs_ph_n
        #act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action"+str(i)) for i in range(len(act_space_n))]
        
        features = env.features()
        p_input = features[p_index]
        #output shape = 2 (connect, which channel to connect)
        p = p_func(p_input, 2, scope="p_func", num_units=num_units)
        p_func_vars = U.scope_vars(U.absolute_scope_name("p_func"))

        # wrap parameters in distribution
        # action == output of mlp model 
        #act_sample = act_pd.sample()
        act_sample = env.action_space()

        #act_input_n = act_ph_n + []
        #act_input_n[p_index] = act_sample
        q_input = features[p_index]
        #if local_q_func:
        #    q_input = tf.concat([obs_ph_n[p_index], act_input_n[p_index]], 1)
        q = q_func(q_input, env.adj_matrix(), 2, scope="q_func", reuse=True, num_units=num_units)[:,0]
        ...
        # target network
        target_p = p_func(p_input, 2, scope="target_p_func", num_units=num_units)
        target_p_func_vars = U.scope_vars(U.absolute_scope_name("target_p_func"))
        update_target_p = make_update_exp(p_func_vars, target_p_func_vars)

        target_act_sample = act_sample
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        q_input = env.features()
        #if local_q_func:
        #    q_input = tf.concat([obs_ph_n[q_index], act_ph_n[q_index]], 1)
        #gat_maddpg (input, adj, num_outputs, scope)
        q = q_func(q_input, env.adj_matrix(), 2,  scope="q_func", num_units=num_units)[:,0]
        ...
        # Create callable functions
        train = U.function(inputs= q_input + [target_ph], outputs=loss, updates=[optimize_expr])
        q_values = U.function(q_input, q)

        # target network
        target_q = q_func(q_input, env.adj_matrix(), scope="target_q_func", num_units=num_units)[:,0]
        target_q_func_vars = U.scope_vars(U.absolute_scope_name("target_q_func"))
        update_target_q = make_update_exp(q_func_vars, target_q_func_vars)

        target_q_values = U.function(q_input, target_q)
